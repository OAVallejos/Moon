//! Etapa 1: Inyección a la Variedad Estable de L1 desde Órbita Terrestre Alta
//! 
...
pub fn validate_ballistic_capture(
    ...
    // Para cruzar L1 hacia la Luna, necesitamos C_J < C_J(L1)
    if cj_actual >= cj_l1 {
        warnings.push(format!(
            "C_J({:.6}) >= C_J(L1)({:.6}): no puede cruzar el cuello hacia la Luna",
            cj_actual, cj_l1
        ));
        feasible = false;
    }
    ...
    if state.x >= xl1 {
        warnings.push(format!(
            "x({:.6}) >= x_L1({:.6}): no está en cuenca terrestre",
            state.x, xl1
        ));
        feasible = false;
    }

    // Para ir hacia la Luna desde la Tierra: vx > 0
    if state[3] <= 0.0 {
        warnings.push("vx <= 0: no se mueve hacia la Luna".to_string());
        feasible = false;
    }
...
pub fn compute_injection(
    ...
    target_cj: f64,       // C_J objetivo en unidades CANÓNICAS (~3.188)
    epsilon_guess: f64,
    tolerance_cj: f64,
    max_iter: usize,
    ...
    // Validación: la nave debe estar en la cuenca terrestre (x < x_L1)
    if orbital.position[0] >= xl1 {
    ...
    // (menor C_J = mayor energía cinética)
    if target_cj >= cj_l1 {
        return Err(InjectionError::JacobiInfeasible(target_cj, cj_l1));
    }
    
    // Permitir hasta 0.05 por debajo de L1
    if target_cj < cj_l1 - 0.05 {
        return Err(InjectionError::JacobiInfeasible(target_cj, cj_l1));
    }

    // Dirección de la variedad INESTABLE (hacia la Luna)
    // Rama correcta: (-1, -k, 0, λ, λk, 0) en el espacio de fase
    ...
    for i in 0..max_iter {
        ...
        // Actualización: ε²_{n+1} = ε²_n - (C_J(ε) - C_J_target) / α
        let eps_sq = eps * eps;
    ...
    // Construir estado de inyección EN UNIDADES CANÓNICAS
